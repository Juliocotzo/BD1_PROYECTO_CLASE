DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `PAIS_nombre_SUB1_PAISR`()
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `FOREIGN_KEY_REFERENCES_PROFESIONAL_id_profesional_`()
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `TEMP_REGION_region_pertenece`()
BEGIN
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `REGION_region_TEMP_DATOS_region`()
BEGIN
